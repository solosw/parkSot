...
export default {
  ...
  data() {
    return {
      colName: [{prop:'id',label:'编号'},{prop:'openId',label:'openId'},{label: "内容",prop: "content"}],
      tableData:[
      ],
      total:1,
      status:null,
  ...
  created(){
    ...
    this.search()
    axios.post("/getFeedCount").then((res)=>{
  ...
  methods: {
    ...
    currentChange(page){
      ...
      axios.post("/getFeedByPage",data).then((res)=>{
    ...
    deleteSlot(){
      const d= JSON.parse(localStorage.getItem("tableRow"))

      console.log(d)
      axios.post("/deleteFeed",{id:d.id}).then((res)=>{
    ...
    sure(){
      axios.post("/addMessage",{openId:this.chooseOrder.openId,content:this.feedContent,type:0,messageId:this.chooseOrder.id}).then((res)=>{
